...
const Printjob = () => {
    ...
    const printHandler = async (id) => {
        if(id != '' && id != 'undefined'){
            printerCapsule();
            if(tokenarray == '' || tokenarray == null ){
                printerCapsule();
                alert('Printer not found, Please try again');
                return ;
            }
            const capsuleData = colorarray[0];
            let selectColor = null;
            if(capsuleData !== '' &&  capsuleData !== 'undefined' ){
                selectColor = colorarray[0].color;
            }

            const jobDetail = await JobDataService.getJob(id);
            if(jobDetail){ 
                let dish2 = null;
                const this_color = colorarray[0];
                if(this_color !== null && this_color !== ''){
                    const papaconfig = {
                        complete: (results, file) => {
                            const dataRows = results.data;
                            dataRows.forEach(row => {
                                if(( (row[0].toLowerCase() ===  jobDetail.shape.toLowerCase()) && row[1].toLowerCase() === selectColor.toLowerCase())){
                                    setDishId('');
                                    dish2= row[2];
                                    setDishId(row[2]);
                                }
                ...
                if(dishId !== null && dishId !== 'undefined' &&  dishId !== ''){
                    ...
                    alert('Missing dish id, please try again');
                }
            }
        }else{
            return ;
    ...
    const printServer = async(thisdishId,jobId) =>{
        let this_dishId = null
        if(process.env.REACT_APP_ENVIRONMENT == 'local'){
            this_dishId  = 3;
        }else{
            this_dishId = thisdishId;
        }

        let printToken = null;
        if(tokenarray !== null && tokenarray !== ''){
            printToken = tokenarray[0];
        }else{
            printToken = apitoken;
        }
        ...
        if(printerlist){
            ...
            await Promise.all(printerlist.map(async (printer) => {
                if(printer.token !== null && printer.token !== '' ){
                    ...
                    //if(printer.last_status === 'Not available'){
                        if(printer.capsule1 !== null && printer.capsule1 !== '' ){
                            tkn.push(printer.token);
                            clr.push({capsule: printer.capsule1, color: 'Green'});

                            //clr.push(printer.capsule1);
                            // setTokenarray(tokenarray => [printer.token,...tokenarray] );
                            // setColorarray(colorarray => [printer.capsule1,...colorarray]);

                        }else if(printer.capsule2 !== null && printer.capsule2 !== '' ){

                            tkn.push(printer.token);
                            clr.push({capsule: printer.capsule2, color: 'Orange'});

                            //clr.push(printer.capsule2);
                            // setTokenarray(tokenarray => [printer.token,...tokenarray] );
                            // setColorarray(colorarray => [printer.capsule2,...colorarray]);

                        }else if(printer.capsule3 !== null && printer.capsule3 !== '' ){

                            tkn.push(printer.token);
                            clr.push({capsule: printer.capsule3, color: 'Purple'});

                            //clr.push(printer.capsule3);
                            // setTokenarray(tokenarray => [printer.token,...tokenarray] );
                            // setColorarray(colorarray => [printer.capsule3,...colorarray]);

                    
                        }else if(printer.capsule4 !== null && printer.capsule4 !== '' ){

                            tkn.push(printer.token);
                            clr.push({capsule: printer.capsule4, color: 'Red'});

                            //clr.push(printer.capsule4);

                            // setTokenarray(tokenarray => [printer.token,...tokenarray] );
                            // setColorarray(colorarray => [printer.capsule4,...colorarray]);

                        }else if(printer.capsule5 !== null && printer.capsule5 !== '' ){

                            tkn.push(printer.token);
                            clr.push({capsule: printer.capsule4, color: 'Yellow'});

                            //clr.push(printer.capsule5);
                            // setTokenarray(tokenarray => [printer.token,...tokenarray] );
                            // setColorarray(colorarray => [printer.capsule5,...colorarray]);
                        }
                    }
                }
            }));
